...
sub UserDotFiles {
    if ( &getGlobalConfig("AccountSecurity","user_dot_files") eq "Y" ) {
        ...
        foreach my $home (@homes) {  
            ...
            if ( -d $home ) {
                my @dotfiles = `$ls -d $home/.[!.]* 1>/dev/null 2>&1`;
                if (($?>>8) != 0 ) {
        ...
        if (&GetDistro =~ '^HP-UX11.(\d*)')  {
            ...
            foreach my $user (@userlist) {
                ...
                if (getpwnam($user)) {
                    ...
                    # lock account
                    &B_System("$passwd -l $user", "$passwd -d $user");
                ...
                if ( -e $tcb) {
                    ...
                    &B_System("$modprpw ¨Cm alock=YES $user", "$modprpw ¨Cm alock=NO $user");
                }
...
sub UnOwnedFiles {
     if ( &getGlobalConfig("AccountSecurity","unowned_files") eq "Y" ) {
        ...
        open UNOWN, "$find / \\\( -nouser -o -nogroup \\\) ! -fstype nfs -print |"; 
        while($file = <UNOWN>) {
            chomp $file;
...
sub NoDotInRootPath {
	...
	# for posix shell  
        my $files = [ &getGlobal("FILE","profile"), &getGlobal("FILE","d.profile") ];
        my $roothome = (getpwnam("root"))[7];
        ...
        for my $file (@$files) {
            ...
            if (@good_path) {
                ...
                if ( $file eq $localprofile ) {
                     my $path_str = join(":", @good_path);
                    $path_str = 'PATH=$PATH:' . $path_str . "\n";
                    &B_append_line($file, "", $path_str);
                }
                # for global profile
                else {
                    my $path_str = join(":", @good_path);
